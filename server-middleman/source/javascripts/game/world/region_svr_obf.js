        ...
        if (-1 !== (data = NBT.nextTag(self))) {
          switch(data.name) {
            ...
            case "ChunkData":
              ...
              if (void 0 !== self.xPos) {
                if (void 0 !== self.zPos) {
                  this.regionSrv.chunkList[1E4 * e + res] = self;
                }
              }
            ;
          }
...
RegionSrv.prototype.getNearestPosition = function(dataAndEvents) {
  ...
  if (void 0 !== this.chunkList[indexLast] && (-1 !== this.chunkList[indexLast] && -2 !== this.chunkList[indexLast])) {
    /** @type {number} */
    ...
    if (0 > l && (l += 16), dataAndEvents = this.chunkList[indexLast].getNearestPosition([getNearestPosition, dataAndEvents[1], l]), false !== dataAndEvents) {
...
RegionSrv.prototype.getBlock = function(pos, deepDataAndEvents, t) {
  ...
  return void 0 !== this.chunkList[message] && (-1 !== this.chunkList[message] && -2 !== this.chunkList[message]) ? (pos -= 16 * imgWidth, 0 > pos && (pos += 16), t -= 16 * englishyPredicate, 0 > t && (t += 16), this.chunkList[message].getBlock(pos, deepDataAndEvents, t)) : {
    id : 0,
    data : 0
  };
};
...
RegionSrv.prototype.updateChunkBlock = function(m2, idx, m1, startAt, hue, deepDataAndEvents, walkers) {
  ...
  if (void 0 !== this.chunkList[accessor]) {
    if (-1 !== this.chunkList[accessor]) {
      if (-2 !== this.chunkList[accessor]) {
        this.chunkList[accessor].updateBlock(m1, startAt, hue, deepDataAndEvents, walkers);
      }
    }
  }
...
RegionSrv.prototype.updateBlock = function(num, startAt, hue, deepDataAndEvents, obj, isXML) {
  ...
  if (void 0 !== this.chunkList[Z]) {
    if (-1 !== this.chunkList[Z]) {
      if (-2 !== this.chunkList[Z]) {
        num -= 16 * t;
        if (0 > num) {
          num += 16;
        }
        ...
        this.chunkList[Z].updateBlock(Math.floor(num), Math.floor(startAt), Math.floor(hue), deepDataAndEvents, obj);
      }
    }
  }
};
        ...
        if (!(-2 === this.chunkList[id])) {
          if (!(true === this.chunkList[id].changed)) {
            if (!(1 !== this.chunkList[id].isInit && 1 !== this.chunkList[id].isInit1)) {
              if (!!(this.chunkList[id].timestamp + 1E4 < min || max)) {
